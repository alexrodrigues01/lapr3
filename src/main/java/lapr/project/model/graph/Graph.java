...
public class Graph<V, E>{
    ...
    public Iterable<Edge<V, E>> edges() {
        Queue<Edge<V, E>> edges = new LinkedList<>();
        for (V vOrig : vertices.keySet()) {
            for (V vDest : vertices.keySet()) {
                if (getEdge(vOrig, vDest) != null) {
                    edges.add(getEdge(vOrig, vDest));
                }
            }
        }
        return edges;
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (V otherVert : vertices.keySet()) {
            if (getEdge(otherVert, vert) != null) {
                edges.add(getEdge(otherVert, vert));
            }
        }
        return edges;
    }
